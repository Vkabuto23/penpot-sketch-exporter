...
- Serialize ordinary groups and nested boards as Sketch `group`. Reserve `shapeGroup` for compound paths that need a shared paint and winding rule; using it for a general container imports as an `Exclude`-like vector in Figma and can hide its child icon, text, or bitmap layers.
- Preserve every subpath of one SVG path in the same painted compound so interior holes remain unfilled. Map Sketch Bézier handles in their native direction: `curveFrom` is outgoing and `curveTo` incoming. For a nested Penpot board with `clipContent`, create a local clipping mask around its children.
...
Run `npm test` after converter changes. The current suite covers ordinary groups, grouped bitmaps, clipped and unclipped nested boards, boolean paths, cubic and quadratic Bézier handles, oval handles, and compound paths with holes. In Figma, deselect the layer before judging whether an icon is visible: blue selection outlines can appear around a transparent object.
